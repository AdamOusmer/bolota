...
// repointed at the blobatar port. No assertion changed.
...
describe("cible de regard", () => {
  test("laisse la pose commander avant que l arrivee ne commence", () => {
  ...
  test("tourne la tete vers la gauche, du cote du panneau", () => {
    // lacet negatif = le bot regarde a gauche
    expect(lookTarget(vise()).yaw).toBe(-TURN);
  });

  test("suit le curseur dans le bon sens sur les deux axes", () => {
  ...
  test("fond le tour a mesure que l arrivee se fait", () => {
...
describe("les deux yeux restent visibles", () => {
   ...
   */
  test("sur les 16 expressions, aux quatre coins de l ecran", () => {
  ...
  test("garde de la marge : le suivi ne va pas jusqu au point de rupture", () => {
...
describe("le tour sur soi-meme", () => {
  test("fait passer les yeux derriere la boule, puis les ramene a gauche", () => {
    /**
...
describe("tour d arrivee sur le site", () => {
   ...
   */
  test("rend la main a la pose en finissant", () => {
    expect(tourLook(TOUR_TIME).mix).toBe(0);
    expect(tourLook(TOUR_TIME + 5).spin).toBe(0);
  });

  test("repose les yeux sur l expression choisie, quelle qu elle soit", () => {
  ...
  test("laisse le bot vivre pendant le tour", () => {
  ...
  test("n impose aucune direction, a aucun moment", () => {
  ...
  test("le tour part d un tour ENTIER, qui est deja le bon angle", () => {
  ...
  test("le tour fait passer les yeux DERRIERE la boule", () => {
   ...
   */
  test("ne peut pas anticiper le roulis, d ou une arrivee sans changement d etat", () => {
...
describe("stabilite du regard entre expressions", () => {
  ...
  test("garde les yeux a la meme hauteur, quelle que soit l humeur affichee", () => {
    /**
  ...
  test("sans pilotage, les expressions gardent bien des hauteurs differentes", () => {
  ...
  test("ne retient que des humeurs a roulis nul, sinon la tete penche", () => {
